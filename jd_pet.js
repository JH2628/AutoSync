...
!(async () => {
  ...
  $.post({ url: `http://zlc1.chaoyi996.com:8880/api/app/frequency-statistic/frequency-statistic?type=5&version=${ver}`, timeout: 10000,}, (err, resp, data) => {
    try {
      if (err) {
  ...
  $.get({ url: 'https://static.zlc1.chaoyi996.com/ccwav/jd_pet.js', timeout: 10000,}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`远程脚本加载失败`)
        } else {
